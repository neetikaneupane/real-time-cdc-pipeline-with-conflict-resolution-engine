...
consumer = KafkaConsumer(
    *all_topics,
    ...
    group_id='cdc-pipeline-consumer',
...
# --- Throughput Server ----------------------------------------------------
class ThroughputHandler(_http.BaseHTTPRequestHandler):
    def do_GET(self):
...
def start_throughput_server():
    server = _http.HTTPServer(('localhost', 9101), ThroughputHandler)
...
def write_conflict_metric(table_name, event_type, strategy=None,
    ...
    """, (
        table_name, event_type, strategy,
        resolution_ms, pk_value, winning_source,
        source_lag_ms, dlq_count
    ))
...
for message in consumer:
    try:
        ...
        is_conflict, sources = check_conflict(
            table_name, pk_value, source, after
        )
